...
def main():
    ...
    # nsml.load(checkpoint='6',  session="KR95459/airush2021-2-5/585")
    ...
    if args.mode == 'train':
        ...
        with torch.no_grad():
            ...
            for i,query in enumerate(queries):
                ...
                for e , idx in enumerate(top_results[0:top_k]): # top k의 copus index들 
                    ## top_results : [3232,53251,5142,311,23,7541,.....]
                    ...
                    # else:
                    #     # 원래점수 고려 labeling
                    #     if cos_scores[top_results[e]] < 0.1:
                    #         pass
                    #     else:
                    #         InputExample_list.append(InputExample(texts=[query, corpus[ top_results[e-1] ]],label= min(cos_scores[top_results[e-1]]+0.2 ,  0.99  ) )  ) 
                    #     if cos_scores[top_results[e]] > 0.9:
                    #         pass
                    #     else:
                    #         InputExample_list.append(InputExample(texts=[query, corpus[ top_results[e] ]],label= max(cos_scores[top_results[e]]-0.2  ,  0.01  )  )  )
                    #     break
                        #원래점수 고려 labeling
                        
                        #극단적 labeling
                        # InputExample_list.append(InputExample(texts=[query, corpus[ top_results[e-(j+1)] ]],label= 0.999  ) )   
                        # InputExample_list.append(InputExample(texts=[query, corpus[ top_results[e+j] ]],label= 0.001  )  )                                
                        ...
                        #극단적 labeling
                        # InputExample_list.append(InputExample(texts=[query, corpus[ top_results[e-1] ]],label= 0.999  ) )   
                        # InputExample_list.append(InputExample(texts=[query, corpus[ top_results[e] ]],label= 0.001  )  )
                        #극단적 labeling
        ...
        for epoch in range(args.epoch):    
            ...
            model.train()
            if warmup == True:
                model = train_iter_STS_warmup(args,epoch+1,batch_size,model,InputExample_list)
                warmup=False
            else:
                model = train_iter_STS(args,epoch+1,batch_size,model,InputExample_list) ## 여기서부터 고치자
            threshold_test(args,model,v_query_sentences,v_query_nv_mids,valid_sentences,valid_nv_mids,epoch+1,gt_list)
            test(args,model,v_query_labels, v_query_sentences,valid_labels,valid_sentences,v_count_dict,epoch+1)
